window.LimnTemplates = {
  ...
  platformer: {
    ...
    fields: [
      ...
      { key: "ground",    label: "Ground color",       type: "color", default: "#4f8" },
      { key: "player",    label: "Player color",       type: "color", default: "#4af" },
      { key: "coin",      label: "Coin color",         type: "color", default: "#fd3" },
    ...
    generate: function(cfg) {
      ...
      return [
        ...
        '  const jf = ' + jump + ';',
        '',
        '  if (display.keys[37] || display.keys[65]) player.x -= sp * dt;',
        '  if (display.keys[39] || display.keys[68]) player.x += sp * dt;',
        '',
        '  if ((display.keys[38] || display.keys[32] || display.keys[87]) && onGround) {',
        '    vy = -jf;',
        '    onGround = false;',
        '  }',
        '',
  ...
  runner: {
    ...
    fields: [
      ...
      { key: "player",   label: "Player color",             type: "color", default: "#4af" },
      { key: "obstacle", label: "Obstacle color",           type: "color", default: "#f44" },
      { key: "speed",    label: "Scroll speed (1-20)",      type: "text",  default: "6" },
      { key: "spawn",    label: "Spawn rate (60-200)",      type: "text",  default: "100" }
    ],
    config: function(cfg) {
      return {
        title: cfg.title,
  ...
  shooter: {
    ...
    fields: [
      ...
      { key: "bg",       label: "Space color",              type: "color", default: "#050510" },
      { key: "player",   label: "Ship color",               type: "color", default: "#4af" },
      { key: "bullet",   label: "Bullet color",             type: "color", default: "#fd3" },
      { key: "enemy",    label: "Enemy color",              type: "color", default: "#f44" },
      { key: "speed",    label: "Move speed",               type: "text",  default: "320" },
      { key: "fireRate", label: "Fire rate (frames)",       type: "text",  default: "12" },
      { key: "spawn",    label: "Enemy spawn (frames)",     type: "text",  default: "90" }
    ],
    config: function(cfg) {
      return {
        title: cfg.title,
    ...
    generate: function(cfg) {
      ...
      return [
        ...
        '  if (player.x + player.width > 800) player.x = 800 - player.width;',
        '',
        '  if ((display.keys[32] || display.keys[38]) && frame % ' + fireRate + ' === 0) {',
  ...
  clicker: {
    ...
    fields: [
      ...
      { key: "bg",       label: "Background",            type: "color", default: "#0d0d2a" },
      { key: "target",   label: "Target color",          type: "color", default: "#4af" },
      { key: "size",     label: "Target size (px)",      type: "text",  default: "60" },
      { key: "lifetime", label: "Lifetime (frames)",     type: "text",  default: "90" },
      { key: "time",     label: "Game time (seconds)",   type: "text",  default: "30" }
    ],
    config: function(cfg) {
      return {
        title: cfg.title,
    ...
    generate: function(cfg) {
      ...
      return [
        ...
        '',
        'const score = new Tctxt("20px","Arial","white",20,40,"left",false,"top","rgba(0,0,0,0.5)",10,4);',
        'score.setText("Score: 0  Time: ' + totalTime + '");',
        'display.add(score);',
        '',
        'let target = null;',
        'let points = 0;',
        ...
        '    const mx = display.x, my = display.y;',
        '    if (mx >= target.x && mx <= target.x + target.width &&',
        '        my >= target.y && my <= target.y + target.height) {',
        '      points += 10;',
        '      score.setText("Score: " + points + "  Time: " + timeLeft);',
        '      target.destroy();',
        '      target = null;',
